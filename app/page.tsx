...
export default async function Home() {
  ...
  // If for some reason profile is still null, use fallback data
  const data = profile || {
    name: "Shiva",
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <header className="sticky top-0 z-50 w-full border-b border-zinc-200 dark:border-zinc-800 bg-white/80 dark:bg-zinc-950/80 backdrop-blur-md">
        <div className="container mx-auto px-4 h-16 flex items-center justify-between">
          <div className="font-bold text-xl tracking-tighter">{data.name}</div>
        ...
        <section className="py-24 md:py-32 container mx-auto px-4 flex flex-col items-center text-center">
          ...
          <div className="flex items-center gap-6 mt-12 text-zinc-500 dark:text-zinc-400">
            {data.socialLinks?.github && (
              <a href={data.socialLinks.github} target="_blank" rel="noreferrer" className="hover:text-zinc-900 dark:hover:text-white transition-colors">
                <FaGithub className="w-6 h-6" />
                <span className="sr-only">GitHub</span>
              </a>
            )}
            {data.socialLinks?.twitter && (
              <a href={data.socialLinks.twitter} target="_blank" rel="noreferrer" className="hover:text-zinc-900 dark:hover:text-white transition-colors">
                <FaTwitter className="w-6 h-6" />
                <span className="sr-only">Twitter</span>
              </a>
            )}
            {data.socialLinks?.linkedin && (
              <a href={data.socialLinks.linkedin} target="_blank" rel="noreferrer" className="hover:text-zinc-900 dark:hover:text-white transition-colors">
          ...
          <div className="container mx-auto px-4 grid md:grid-cols-2 gap-12 items-center">
            <div>
              ...
              <div className="prose prose-zinc dark:prose-invert text-lg text-zinc-600 dark:text-zinc-400">
                <p className="whitespace-pre-wrap">{data.aboutText}</p>
              </div>
            </div>
            <div className="grid grid-cols-2 gap-6">
              <div className="bg-white dark:bg-zinc-950 p-6 rounded-2xl border border-zinc-200 dark:border-zinc-800">
                <div className="text-4xl font-bold mb-2">{data.yearsExperience || "0"}</div>
                <div className="text-sm font-medium text-zinc-500">Years Experience</div>
              </div>
              <div className="bg-white dark:bg-zinc-950 p-6 rounded-2xl border border-zinc-200 dark:border-zinc-800">
                <div className="text-4xl font-bold mb-2">{data.projectsDelivered || "0"}</div>
              ...
              <div className="bg-white dark:bg-zinc-950 p-6 rounded-2xl border border-zinc-200 dark:border-zinc-800 col-span-2">
                <div className="text-4xl font-bold mb-2">{data.technologiesMastered || "0"}</div>
